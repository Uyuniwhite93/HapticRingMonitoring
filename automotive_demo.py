...
class AutomotiveDisplay:
        ...
        for button in self.buttons:
            if button["rect"].collidepoint(pos):
                ...
                if button["name"] == "WindDown":
                    ...
                    self.update_wind_display()
                    print(f"🌪 WIND SET → Level {self.wind_level} (No haptic)")
                    return None  # 햅틱 없음
                else:
                    # 일반 버튼 토글 + 강력한 RA burst!
                    button["active"] = not button["active"]
                    
                    # 버튼 클릭 RA burst 추가! (클릭감)
                    self.trigger_button_click_ra_burst()
                    
                    if button["active"]:
                        print(f"💥 BUTTON CLICK: {button['name']} ACTIVATED")
                    else:
                        print(f"💥 BUTTON CLICK: {button['name']} DEACTIVATED")
    ...
    def handle_mouse_release(self):
        """마우스 해제 처리 - main.py 방식: SA 입력 리셋 + RA burst"""
        # main.py 방식: mouseReleaseEvent에서 SA 입력을 0으로 설정
        self.spike_encoder.update_sa_input(0.0)
        print(f"🔵 SA STOP: Mouse released! (0.0A)")
        
        # 해제 시 RA burst (순간적 촉각)
        self.trigger_click_release_ra_burst()
    ...
    def handle_mouse_move(self, pos):
        ...
        if self.hovered_button != self.prev_hovered_button:
            if self.hovered_button:
                # WindStep 버튼은 hover 햅틱 없음
                if not self.hovered_button.get("wind_step"):
                    # 새 버튼 진입 (일반 버튼만)
                    self.trigger_button_enter_burst()
                    self.start_sa_hover()
                    print(f"🔵 ENTERED: {self.hovered_button['name']}")
                else:
                    print(f"🔵 ENTERED: {self.hovered_button['name']} (No haptic)")
            elif self.prev_hovered_button:
                # WindStep 버튼은 hover 햅틱 없음  
                if not self.prev_hovered_button.get("wind_step"):
                    # 버튼 이탈 - SA 완전 리셋 (일반 버튼만)
                    self.trigger_button_exit_burst()
                    self.stop_sa_hover()
                    self.spike_encoder.update_sa_input(0.0)  # 확실한 리셋
                    print(f"🔴 EXITED: {self.prev_hovered_button['name']}")
                else:
                    print(f"🔴 EXITED: {self.prev_hovered_button['name']} (No haptic)")
    ...
    def trigger_click_start_ra_burst(self):
        """클릭 시작 시 RA burst (순간적 촉각)"""
        ...
        if ra_fired:
            ...
            print(f"💥 RA BURST: Click START!")
    
    def trigger_click_release_ra_burst(self):
        """클릭 해제 시 RA burst (순간적 촉각)"""
        # 클릭 해제용 RA burst (클릭보다 약간 약함)
        ra_fired, _, _, ra_vu = self.spike_encoder.step(
        ...
        if ra_fired:
            ...
            print(f"💫 RA BURST: Click RELEASE!")
    
    def trigger_button_click_ra_burst(self):
        """버튼 클릭 시 강력한 RA burst - 클릭감 제공"""
        # 초강력한 RA burst (버튼 클릭감)
        ra_fired, _, _, ra_vu = self.spike_encoder.step(
            mouse_speed=12000.0,     # 더 높은 속도
            avg_mouse_speed=12000.0,
            material_roughness=2.0,   # 더 거친 재질
            mouse_pressed=True
        )
        
        if ra_fired:
            # 강력한 버튼 클릭 RA 사운드
            button_click_sound = self.haptic_renderer.create_sound_object(220, 60, 1.0, fade_out_ms=3)
            if button_click_sound:
                self.audio_player.play_sound(button_click_sound, channel_id=3, volume=1.0)
            print(f"🔥 BUTTON RA BURST: Strong click feedback!")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:  # 왼쪽 클릭
                        ...
                        # main.py 방식: 클릭 시 SA 입력 시작 (지속)
                        self.spike_encoder.update_sa_input(12.0)  # main.py click_mag
                        print(f"🔴 SA START: Click pressed (12.0A)")
                        
                        # 클릭 시작 RA burst (순간적 촉각)
                        self.trigger_click_start_ra_burst()
